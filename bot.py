...
async def cmd_staff(message: Message) -> None:
    ...
    if not employees:
        await message.answer("📋 Список работников пуст. Используйте /add_employee")
        return

    await message.answer(f"👥 <b>Работники ({len(employees)}):</b>", parse_mode="HTML")
    for emp in employees:
        emp_uid, username, fio, position, area, registered = emp
...
async def _submit_complaint(
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute(
            "INSERT INTO complaints (user_id, username, fio, address, description, media_file_id, media_type)"
            " VALUES (?,?,?,?,?,?,?)",
            (uid, username, fio, address, description, media_file_id, media_type),
        )
