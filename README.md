# 🏠 ЖКХ Toolkit - Telegram Bot + Web Panel
...
- **Работник**: 6-значный код из команды `/link_account`
...
# Запустить веб-панель (в другом терминале)
...
| Проблема | Решение |
|----------|---------|
| Бот не отвечает | Проверьте `BOT_TOKEN` в `.env`, логи: `docker-compose logs bot` |
| Веб-панель 500 | Проверьте логи: `docker-compose logs web` |
| Уведомления не приходят | Проверьте `BOT_TOKEN` в `.env` для веб-сервиса |
| Код работника не принимается | Код действует 10 минут, запросите новый `/link_account` |
